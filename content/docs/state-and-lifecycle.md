...
เพื่อจะพัฒนาในส่วนนี้ เราจำเป็นต้องเพิ่ม "สถานะ"("state") เข้าไปใน `Clock` คอมโพเนนท์

สถานะจะมีความคล้ายคลึงกับ props แต่จะมีความเป็นส่วนตัว(private) และสามารถควบคุมได้เต็มที่จากคอมโพเนนท์
...
3. ย้ายโค๊ดในตัวของฟังค์ชั่นไปไว้ใน `render()` เมธอด
...
`Clock` จะถูกประกาศในรูปแบบของคลาสแทนรูปแบบของฟังค์ชั่น

สำหรับเมธอด `render` จะถูกเรียกทุกครั้งที่มีการปรับปรุงเกิดขึ้น แต่ถ้าหากเราใช้ `<Clock />` ในโหนดDOM เดิม จะมีเพียงอินสแตนเดียวของคลาส `Clock` เท่านั้นที่จะถูกสร้างขึ้นมาใช้ นี้ทำให้เรามีความสามารถเพิ่มเติมเช่น สถานะภายใน(local state) และเมธอดของวงจรขีวิต (lifecycle methods)
...
ซึ่งจะมีการใช้ `this.setState()` เพื่อกำหนดการ(schedule)ปรับปรุงสถานะของคอมโพเนนท์:
...
4) ในทุกๆวินาทีเบราว์เซอร์จะทำการเรียกเมธอด `tick()` จากนั้นคอมโพเนนท์ `Clock` จะกำหนดการปรับปรุงส่วนแสดงผลโดยการเรียกใช้ `setState()` ด้วยออบเจ็คที่ประกอบด้วยข้อมูลเวลาปัจจุบัน ในการเรียกใช้ `setState()` จะทำให้ React รู้ว่ามีการเปลี่ยนแปลงของสถานะภายในและจะมีการเรียกใช้เมธอด `render()` อีกครั้ง เพื่อจะคำนวนผลลัพธ์และจะปรับปรุงการแสดงผลบนจอภาพอย่างไร ซึ่งในขณะนี้ `this.state.date` ที่อยู่ในเมธอด `render()` จะมีความแตกต่างจากเดิม และการแสดงผลลัพธ์นั้นจะแสดงเวลาที่ถูกปรับปรุงแล้ว และ React จะปรับปรุง DOM ตามไปด้วย
...
และนี้เป็นสาเหตุทำให้ สถานะ เป็นส่ิงที่อยู่ภายใน หรือ ถูกห่อหุ้มเอาไว้ และไม่สามารถเข้าถึงได้จากคอมโพเนนท์อื่น ยกเว้นคอมโพเนนท์ที่เป็นเจ้าของสถานะนั้น
...
คอมโพเนนท์ `FormattedDate` จะได้รับ `date` ในรูปแบบของ props และไม่สามารถรู้ได้ว่าแหล่งที่มา ว่ามาจากสถานะของ `Clock` หรือมาจาก props ของ `Clock` หรือเป็นค่าที่พิมพ์ด้วยมือ:
